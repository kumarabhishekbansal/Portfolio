...
import { sendachievementpage,reset } from "../../components/Reducer/AuthSlice";
const Achievements = () => {
  ...
  const handleSubmit=(e)=>{
    e.preventDefault();
    dispatch(sendachievementpage(fields));
}
    ...
    try {
      fetch("https://api.cloudinary.com/v1_1/do2yjfyfe/image/upload", {
        ...
        .then((data) => {
          console.log(data);
          setslide(data.url);
          setfields({ ...fields, achievement_certificates: data.url });
        })
  ...
  useEffect(()=>{
    if(isSuccess)
    {
      ...
      toast.success("achievementpage updated..");
      reset();
    }else if(message)
    {
      console.log(message);
      toast.error("achievementpage can not be updated..");
      reset();
    }
    reset();
  ...
  return (
    <>
      <div className="container">
        <h1 className="text-info text-center">Achievement Page</h1>
        <form onSubmit={handleSubmit}>
          {/* Introduction */}
          <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">
              Enter some information about Your Achievements
            </label>
            <textarea
              class="form-control"
              id="achievement_subtitle"
              name="achievement_subtitle"
              rows="3"
              onChange={handleInput}
              value={fields.achievement_subtitle}
            ></textarea>
          </div>
          ...
          <div class="mb-3">
            ...
            <textarea
              ...
              rows="3"
              name="achievement_intro"
              onChange={handleInput}
              value={fields.achievement_intro}
            ></textarea>
          </div>
